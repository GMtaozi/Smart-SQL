...
【示例】
Q: 入库金额大于1000的产品
A: SELECT DISTINCT ird.product_name FROM inbound_record ir JOIN inbound_record_detail ird ON ir.id = ird.inbound_record_id WHERE ir.total_amount > 1000

Q: 每个类别的订单数量
A: SELECT category, COUNT(*) as cnt FROM orders GROUP BY category

Q: 今天之前入库的产品
A: SELECT * FROM inbound_record WHERE create_time < CURRENT_DATE

Q: 最近7天内的订单
A: SELECT * FROM orders WHERE create_time >= CURRENT_DATE - INTERVAL '7 days'
